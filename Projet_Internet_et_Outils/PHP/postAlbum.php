...
<form action="" method="get" id="artiste">
    ...
    <label for="artiste">Qui est l'artiste de cet album ?</label>
...
if (isset($artiste)){
    if ($artiste != 'rien'){
        echo ('<form action ="finAlbum.php" method="get" onsubmit="return validerFormulaire()">
        <label for="album">Choisissez un album de cet artiste </label>
        <select name="album" id="album">
        <option value="rien">--Veuillez choisir un album--</option>');
        ...
        $result = $mysqli->query("SELECT nom FROM album WHERE artiste='$artiste' ORDER BY nom ASC");

        while($row = $result->fetch_assoc()) {
        echo "<option value=\"" . $row['nom'] . "\" name=\"album\">" . $row['nom'] . "</option>";
        }
        ...
        echo '  var albumSelect = document.getElementById(\'album\').value;';
        echo '  if (albumSelect === \'rien\') {';
        echo '    alert(\'Veuillez sélectionner un album.\');';
        echo '    return false;';
        echo '  }';
        echo '  return true;';
        echo '}';
