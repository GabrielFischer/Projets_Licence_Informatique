...
if(!isset($_GET["morceau"])){
    ...
    if(!isset($_GET["id"])){
        ...
        $morceau_min = strtolower($morceau);
        session_start();
        $connexion=mysqli_connect("localhost","root","","moosik");
        if ($connexion){
            $recherche=mysqli_query($connexion,"SELECT * FROM morceau WHERE LOWER(nom)='$morceau_min'");
            if(mysqli_num_rows($recherche)!=0){
                ...
                echo("Oops, nous n'avons pas trouvé de morceau à ce nom, veuillez vérifiez l'orthographe");
                //AFFICHER LA BARRE DE RECHERCHE
            }
        }
    }else{
        ...
        $connexion=mysqli_connect("localhost","root","","moosik");
        if ($connexion){
            $recherche=mysqli_query($connexion,"SELECT * FROM morceau WHERE id='$id'");
            if(mysqli_num_rows($recherche)!=0){
