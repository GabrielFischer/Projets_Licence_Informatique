...
if(isset($_GET['morceau'])){
    $morceau=htmlspecialchars($_GET["morceau"]);
}
if (isset($morceau)&&isset($artiste)){
    ...
    if(isset($_SESSION['mail'])){
        ...
        $userid = $tmp['id'];
        $exist = mysqli_query($connexion, "SELECT * FROM morceau WHERE nom='$morceau' AND artiste='$artiste';");
        if (mysqli_num_rows($exist)!=0){

            $result = mysqli_query($connexion, "SELECT * FROM posts WHERE userId='$userid' AND titre='$morceau' AND type='avis_morceau' AND artiste='$artiste';");

            if(mysqli_num_rows($result)!=0){
                $row=mysqli_fetch_assoc($result);
                $note_enr=$row["etoiles"];
                $avis_enr=$row["texte"];
                echo("<h2>Vous avez déjà noté cet morceau</h2><br>");
                ...
                echo("<a href='modifmorceau.php?morceau=$morceau&artiste=$artiste'>Modifier</a> ");
                ...
                if (isset($morceau)&&isset($artiste)&&isset($note)&&isset($texte)){
                    $connexion = mysqli_connect("localhost", "root", "", "moosik");
                    session_start();
                    if($connexion->connect_error) {
                        die("Erreur de connexion : " . $connexion->connect_error);
                    ...
                    if(isset($_SESSION['mail'])){
                        $result = mysqli_query($connexion, "SELECT * FROM posts WHERE userId='$userid' AND titre='$morceau' AND artiste='$artiste';");
                
                        if(mysqli_num_rows($result)==0){
                            
                            mysqli_query($connexion, "INSERT INTO posts (type,Titre,Artiste,userId,etoiles,texte) VALUES('avis_morceau','$morceau','$artiste','$userid','$note','$texte')");
            ...
            echo("Oops, il semblerait que nous n'avons pas réussi à trouver ce morceau.");
        }
    }else{
        header("Location:Profil/LogIn.php");
    }
