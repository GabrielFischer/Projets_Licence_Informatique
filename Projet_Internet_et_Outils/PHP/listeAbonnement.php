    ...
    if (isset($_SESSION["id"])){
        if(isset($_GET["userid"])){
            if($_SESSION["id"]==$_GET["userid"]){
                echo("<h2> Vos abonnements: </h2>");
                $liste=mysqli_query($connexion,"SELECT * FROM abonnement WHERE id_abonne='{$_GET['userid']}';");
                if (mysqli_num_rows($liste)>0){
                    while($row=mysqli_fetch_assoc($liste)){
                        $abonnement=$row["id_abonnement"];
                        $name=mysqli_query($connexion,"SELECT * FROM users WHERE id='$abonnement';");
